...
export default function OpportunityForm({ setShowOpportunityForm }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.status === 201) {
        setSuccessMessage("opportunitie created successfully!"); // Set success message
        setTimeout(() => {
    ...
    <div className="fixed inset-0 flex justify-center items-center bg-gray-900 bg-opacity-50">
      <div className="bg-white rounded-lg shadow-lg w-full max-w-4xl h-[100vh] flex flex-col">
        <div className="flex gap-4 items-center justify-between p-4 md:p-5 border-b rounded-t">
          <image alt="header image" width="44" height="44" class="w-10 h-9" src="/images/employee_contact.2d215fd6.svg"/>
          <h2 className="text-2xl font-semibold">Create opportunitie</h2>
          <button type="button" onClick={() => setShowOpportunityForm(false)} class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-4">
            ...
            <div className="flex flex-col">
              ...
              >
                ...
                <option value="Demo attened">Demo attened</option>
                <option value="Lost Opportunity">Lost Opportunity</option>
              </select>
            </div>

            <div className="flex flex-col">
              <label className="font-medium text-base">CC</label>
              <input
                type="text"
                name="CC"
                value={formData.CC}
